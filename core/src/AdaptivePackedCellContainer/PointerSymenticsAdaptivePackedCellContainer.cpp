...
namespace PredictedAdaptedEncoding
{
    ...
    ) noexcept
    {
        ...
        while (curent_tries++ < max_claim_attempts)
        {
            ...
            if (curent_ptr_position == SubClassesOfMode32::REL_OFFSET_HEAD_PTR)
            {
                head_idx = probable_idx;
                tail_idx = (probable_idx + 1) % PayloadCapacityFromHeader();
            }
            else if (curent_ptr_position == SubClassesOfMode32::RELOFFSET_TAIL_PTR)
            {
    ...
    void PointerSymenticsAdaptivePackedCellContainer::RetireAcquiredPointerPair(const AcquirePairedPointerStruct& acquired_paired_pointer_struct) noexcept
    {
        ...
        packed64_t idle32 = PackedCell64_t::MakeInitialValidPackedCell(PackedMode::VALUE32);
            ...
            if (head_locality == PackedCellLocalityTypes::ST_IDLE && tail_locality == PackedCellLocalityTypes::ST_IDLE)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        meta16_t strl_tail = PackedCell64_t::MakeInCellMetaForMode_32t(PriorityPhysics::IDLE, PackedCellOwnership::ADAPTIVE_PACKED_CELL_CONTAINER, PackedCellLocalityTypes::ST_PUBLISHED, rel_mask_with_ptrflag, SubClassesOfMode32::RELOFFSET_TAIL_PTR);
                        ...
                        meta16_t strl_head = PackedCell64_t::MakeInCellMetaForMode_32t(PriorityPhysics::IDLE, PackedCellOwnership::ADAPTIVE_PACKED_CELL_CONTAINER, PackedCellLocalityTypes::ST_PUBLISHED, rel_mask_with_ptrflag, SubClassesOfMode32::REL_OFFSET_HEAD_PTR);
